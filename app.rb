...
class TimeEntryTest < Minitest::Test
  ...
  def test_time_entries_with_client_name
    ...
    worked_on, duration, time_entries.created_at, time_entries.updated_at
    FROM time_entries
    INNER JOIN projects
  ...
  def test_total_hours_per_client
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT SUM(duration), clients.name
    FROM time_entries
    INNER JOIN projects
    ON time_entries.project_id = projects.id
    INNER JOIN clients
  ...
  def test_client_lupe_worked_most_hours_for
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT SUM(duration), developers.name, clients.name
    FROM developers
    ...
    INNER JOIN clients
    ON projects.client_id = clients.id WHERE developers.name = 'Mrs. Lupe Schowalter'
    GROUP BY clients.id
  ...
  def test_list_all_client_names_with_their_project_names
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT clients.name, projects.name
    FROM clients
    LEFT JOIN projects
  ...
  def test_find_all_developers_with_no_comments
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT developers.name, comments.comment
    FROM developers
    LEFT JOIN comments
  ...
  def test_find
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT SUM(duration), developers.name
    FROM time_entries
    ...
    ORDER BY SUM(duration) LIMIT 1"
    assert_equal [[0, "Ms. Tremayne Kuhn"]], db.execute(sql)
  end
end
