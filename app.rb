...
class TimeEntryTest < Minitest::Test
  ...
  def test_time_entries_with_client_name
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT clients.name, time_entries.id, developer_id, project_id,
    worked_on, duration, time_entries.created_at, time_entries.updated_at FROM
    ...
    assert_equal ["Goodwin Group", 500, 50, 30, "2014-09-22", 4,
    "2015-07-14 16:15:19.621743", "2015-07-14 16:15:19.621743"], db.execute(sql).last
  end

  def test_all_Ohio_Sheep_developers
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT developers.name FROM developers INNER JOIN group_assignments
    ON developers.id = group_assignments.developer_id INNER JOIN groups ON
    group_assignments.group_id = groups.id WHERE groups.name='Ohio sheep'"
    assert_equal [["Bruce Wisoky Jr."], ["Eli Wunsch MD"], ["Reyes Vandervort IV"]], db.execute(sql)
  end

  def test_total_hours_per_client
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT SUM(duration), clients.name FROM time_entries INNER JOIN
    projects ON time_entries.project_id = projects.id INNER JOIN clients ON
    projects.client_id = clients.id GROUP BY clients.id"
    assert_equal [238, "Goodwin Group"], db.execute(sql).last
  end

  def test_client_lupe_worked_most_hours_for
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT SUM(duration), developers.name, clients.name FROM developers
    INNER JOIN time_entries ON developers.id = time_entries.developer_id INNER
    JOIN projects ON time_entries.project_id = projects.id INNER JOIN clients ON
    projects.client_id = clients.id WHERE developers.name = 'Mrs. Lupe Schowalter'
  ...
  def test_list_all_client_names_with_their_project_names
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT clients.name, projects.name FROM clients LEFT JOIN projects
  ...
  def test_find_all_developers_with_no_comments
    db = SQLite3::Database.new "time_entries.sqlite3"
    sql = "SELECT developers.name, comments.comment FROM developers LEFT JOIN
